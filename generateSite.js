// generateSite.js
// articles.json se poora static news website generate karta hai:
// homepage, category pages, individual article pages, aur sitemap.xml
...
// ---------- CONFIG: apna branding yahan badal sakte ho ----------
const SITE_NAME = "Pulse Wire";
const SITE_TAGLINE = "Signal over noise.";
const SITE_URL = "https://example.com"; // hosting milte hi apna real domain daal dena
...
// ---------- CSS (design tokens: paper/ink/signal/live, Fraunces+Inter+Plex Mono) ----------
const CSS = `
:root {
  --paper: #F7F7F5;
  --ink: #14161C;
...
function headerHtml(categories, articles) {
  const navLinks = categories
    .map((c) => `<a href="/category/${slugify(c)}.html">${escapeHtml(c)}</a>`)
    .join("");
  return `
${tickerHtml(articles)}
...
function buildSitemap(articles) {
  const urls = articles
    .map((a) => `<url><loc>${SITE_URL}/article/${a.slug}.html</loc></url>`)
    .join("\n");
...
async function main() {
  ...
  categories.forEach((cat) => {
    fs.writeFileSync(
      path.join(OUTPUT_DIR, "category", `${slugify(cat)}.html`),
      buildCategoryPage(cat, articles, categories),
      "utf-8"
    );
  });
  ...
  articles.forEach((a) => {
    fs.writeFileSync(
      path.join(OUTPUT_DIR, "article", `${a.slug}.html`),
      buildArticlePage(a, categories, articles),
      "utf-8"
    );
  });
